/**
 * Test script to create sample abandoned carts for testing
 * Run: npx ts-node src/scripts/create-test-carts.ts
 */
...
async function createTestCarts() {
  try {
    ...
    if (!user) {
      user = await prisma.user.create({
        data: {
          ...
          name: 'Test User',
          password: 'test123', // hashed in production
        },
      })
    ...
    const carts = await Promise.all([
      ...
      prisma.cart.upsert({
        ...
        create: {
          ...
          totalValue: 199.99,
          abandonedAt: new Date(Date.now() - 10 * 60 * 1000), // 10 minutes ago
        },
      }),
    ])
    ...
    carts.forEach((cart) => {
      console.log(`   - Cart ${cart.id}: ${cart.customerName} ($${cart.totalValue})`)
    })

    console.log('\n✅ Test data created successfully!')
    console.log('\nNow you can:')
    console.log('1. Visit http://localhost:3000/api/queue/init')
    console.log('2. Trigger cart processing: POST /api/jobs/process-carts')
    console.log('3. Check logs for email sends')
  } catch (error) {
