...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!razorpayAdapter.verifyWebhookSignature(body, headers)) {
      return NextResponse.json({ error: 'Invalid signature' }, { status: 401 })
    }

    const event = razorpayAdapter.parseEvent(body, headers)
    if (!event) {
    ...
    console.error('Razorpay payment webhook error:', error)
    return NextResponse.json({ received: true }, { status: 200 })
  }
}
